...
function placeFormatInfo(m, maskIdx) {
  ...
  for (let i = 0; i <= 5; i++) setFunction(m, 8, i, (bits >> i) & 1);
  setFunction(m, 8, 7, (bits >> 6) & 1);
  setFunction(m, 8, 8, (bits >> 7) & 1);
  setFunction(m, 7, 8, (bits >> 8) & 1);
  for (let i = 9; i < 15; i++) setFunction(m, 14 - i, 8, (bits >> i) & 1);

  // вторая копия: биты 0–6 — левый нижний столбец, биты 7–14 — правая верхняя строка
  for (let i = 0; i < 7; i++) setFunction(m, m.size - 1 - i, 8, (bits >> i) & 1);
  for (let i = 7; i < 15; i++) setFunction(m, 8, m.size - 15 + i, (bits >> i) & 1);
