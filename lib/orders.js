...
/** Почта гостя: необязательна, но если указана — должна быть похожа на адрес. */
function normalizeEmail(value) {
  const email = (value || '').toString().trim().toLowerCase().slice(0, 120);
  if (!email) return '';
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(email)) throw new OrderError('bad_email', 'Проверьте адрес почты');
  return email;
}

/** Гость может подписаться на письма уже после оформления — со страницы заказа. */
function setEmail(order, value, lang) {
  if (['picked_up', 'cancelled', 'no_show'].includes(order.status)) {
    throw new OrderError('finalized', 'Заказ уже завершён');
  }
  const email = normalizeEmail(value);
  if (!email) throw new OrderError('bad_email', 'Укажите адрес почты');
  order.email = email;
  if (lang === 'en' || lang === 'ru') order.lang = lang;
  store.save({ type: 'order_email', venueId: order.venueId, orderId: order.id });
  return order;
}
...
module.exports = {
  OrderError, STATUSES, create, setStatus, advance, markArrived, rate, setEmail, normalizeEmail,
