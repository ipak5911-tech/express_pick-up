...
function travelMinutes(from, venue, ts = Date.now(), mode = 'car') {
  // У заведения из старой базы координат может не быть: тогда дорога просто
  // не учитывается, а не роняет запрос слотов.
  if (!from || !venue || !venue.location ||
      !Number.isFinite(venue.location.lat) || !Number.isFinite(venue.location.lon)) {
    return null;
  }
