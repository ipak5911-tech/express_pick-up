...
function availableSlots(venue, allOrders, cart, now = Date.now(), travelMinutes = 0) {
  ...
  // Ближайший доступный слот и ближайший, который открылся бы без учёта дороги:
  // гость должен видеть не только отказ, но и что делать дальше.
  const firstAvailable = out.find(r => r.available) || null;
  const firstIfNearby = travelMinutes > 0
    ? (out.find(r => r.available || r.reason === 'too_far') || null)
    : firstAvailable;

  return {
    slots: out,
    firstAvailable: firstAvailable ? { start: firstAvailable.start, label: firstAvailable.label } : null,
    firstIfNearby: firstIfNearby ? { start: firstIfNearby.start, label: firstIfNearby.label } : null,
    travelMinutes,
    workSeconds: work,
...
/**
 * Живой статус заведения: открыто ли прямо сейчас и принимает ли заказы.
 *
 * «Открыто» и «принимает заказы» — разные вещи: кухня может работать, но все
 * ближайшие слоты выбраны, а до закрытия может остаться меньше, чем
 * минимальный запас времени на приготовление.
 */
function venueStatus(venue, allOrders, now = Date.now()) {
  const settings = venue.settings;
  const openNow = withinServiceHours(now, settings);
  ...
  const opensToday = bound(settings.serviceHours.from);
  const closesToday = bound(settings.serviceHours.to);

  const { slots } = availableSlots(venue, allOrders, [], now);
  const free = slots.filter(s => s.available);
  const closesInMinutes = Math.round((closesToday.getTime() - now) / MIN);

  return {
    openNow,
    opensAt: settings.serviceHours.from,
    closesAt: settings.serviceHours.to,
    // предупреждаем, пока ещё можно успеть оформить заказ
    closingSoon: openNow && closesInMinutes <= Math.max(30, settings.minLeadMinutes * 2),
    closesInMinutes: openNow ? closesInMinutes : null,
    opensInMinutes: !openNow && now < opensToday.getTime()
      ? Math.round((opensToday.getTime() - now) / MIN)
      : null,
    acceptingOrders: openNow && free.length > 0,
    freeSlots: free.length,
    nextSlotAt: free.length ? free[0].start : null,
    nextSlotLabel: free.length ? free[0].label : null
  };
}
...
module.exports = {
  ...
  buildLedger, allocate, availableSlots, reserve, forecast, withinServiceHours, venueStatus
};
